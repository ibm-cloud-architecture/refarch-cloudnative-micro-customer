...
1. Start the service using the following, make sure that the parameters are replaced correctly:
...
```
# curl -H "Auhtorization: Bearer <JWT>"  http://localhost:8080/micro/customer
...
10. Verify the deployment
    ...
    The customer service can be exposed as an ingress resource, which allows external access to it from the internet via the ingress controller.  This exposes the Customer microservice REST API at `http://<cluster_name>.<region>.containers.mybluemix.net/micro/customer`.  To enable, create the ingress resource:
    
    ```
    # kubectl create -f kubernetes/customer-ingress.yaml
    ```

    When finish verifying the deployment is successful and working as expected, delete the ingress resource so it is no longer external exposed:
    ```
